...
export default class ActivityOder extends Component {
	constructor(props) {
		super(props);
		this.state = {
			ActivityOder: [],
	...
	tabRow() {
		return this.state.ActivityOder.map(function (object, i) {
	...
	exportPDF = () => {
        ...
        const headers = [["email", "activity","price", "Qty","status"]];
    
        const data = this.state.ActivityOder.map(elt=> [elt.email, elt.activity,  elt.price,elt.Qty, elt.status]);
	...
	render() {
		return (
			<div className='adminVehicleProfile'>
				...
				<br /> <h3 align='center'>Activity Oder Management</h3>
